...
export class NodeLoader {
  ...
  async load(node) {
    ...
    try {
      ...
      let urlOctree = this.url.pointCloudFilesObject.octree; //`${this.url}/../octree.bin`;
  ...
  async loadHierarchy(node) {
    ...
    let hierarchyPath = this.url.pointCloudFilesObject.hierarchy; //`${this.url}/../hierarchy.bin`;
...
export class OctreeLoader {
  ...
  static async load(url) {
    ...
    // get the blob for metadata.json and give its url
    const blobUrl = await url.fetchObject(url.pointCloudFilesObject.metadata);
    let response = await fetch(blobUrl);
